...
file_path = '/Users/ashinwalpola/Documents/University/HAW Hamburg/IE_3/AD-Dierks/Lab/Lab02/BottomUpMerge_Sort_testing.txt'
...
# Plot Number array (N) vs. Computation time with shaded area for Range
plt.figure()
plt.plot(N, average_computation_time, marker='o', label='Average Computation Time')
plt.fill_between(N, low_computation_time, high_computation_time, alpha=0.2, label='Range')
plt.xlabel('Array Size (N)')
plt.ylabel('Computation Time (milliseconds)')
plt.title('Bottom Up Merge Sort Computation Time vs. Array Size')
plt.grid(True)
plt.xscale('log')
plt.yscale('log')
plt.legend()
...
plt.fill_between(average_computation_time, low_comparisons, high_comparisons, alpha=0.2, label='Comparisons Range')
plt.plot(average_computation_time, average_exchanges, marker='^', label='Average Exchanges')
plt.fill_between(average_computation_time, low_exchanges, high_exchanges, alpha=0.2, label='Exchanges Range')

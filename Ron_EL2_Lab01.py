...
for Vgs in Vgs_values:
    ...
    # Calculate rON for each curve
    rON = np.diff(Id) / np.diff(Uds_values)
    max_idx = np.argmax(rON)  # Index of maximum rON
    max_rON = rON[max_idx]  # Maximum rON value
    plt.annotate(f'rON = {max_rON:.2f}', (Uds_values[max_idx], Id[max_idx]), textcoords="offset points", xytext=(0,10), ha='center')
